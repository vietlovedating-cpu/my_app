...
class EditAnnualIncomePage extends StatefulWidget {
  ...
  const EditAnnualIncomePage({
    super.key,
    required this.languageCode,
  });

  @override
  State<EditAnnualIncomePage> createState() => _EditAnnualIncomePageState();
}

class _EditAnnualIncomePageState extends State<EditAnnualIncomePage> {
  String? _selectedKey;
  ...
  Future<void> _loadCurrentData() async {
    try {
      ...
      final data = doc.data() ?? {};
      final value = (data['annualIncome'] ?? '').toString();
    ...
    if (_selectedKey == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            _tr('Vui lòng chọn lương năm', 'Please choose annual income'),
          ),
        ),
      );
      return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          _tr('Sửa lương năm', 'Edit annual income'),
          style: const TextStyle(
