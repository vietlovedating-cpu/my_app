...
class EditMaritalStatusPage extends StatefulWidget {
  final String languageCode;

  const EditMaritalStatusPage({
    super.key,
    required this.languageCode,
  });

  @override
  State<EditMaritalStatusPage> createState() => _EditMaritalStatusPageState();
}

class _EditMaritalStatusPageState extends State<EditMaritalStatusPage> {
  ...
  Future<void> _loadCurrentData() async {
    try {
      ...
      final data = doc.data() ?? {};
      final value = (data['maritalStatus'] ?? '').toString();
    ...
    if (_selectedValue == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            _tr('Vui lòng chọn tình trạng hôn nhân', 'Please choose marital status'),
          ),
        ),
      );
      return;
...
// Lưu tình trạng hôn nhân
await userRef.set({
  'maritalStatus': _selectedValue,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          _tr('Sửa hôn nhân', 'Edit marital status'),
          style: const TextStyle(
